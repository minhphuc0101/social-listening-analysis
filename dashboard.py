...
# =============================================================
    ...
    t_ch_groups, t_ch_macro = st.tabs([
        "🏢 Xếp hạng sắc thái theo Từng Trang & Hội Nhóm (Page Benchmark)",
        ...
        if not df.empty:
            ...
            with col_gkpi1:
                ...
                <div class="dashboard-card" style="margin-bottom:0.75rem; padding:1rem;">
                    <div style="font-size:0.75rem; font-weight:700; color:#64748B; text-transform:uppercase;">🏢 Trang & Hội Nhóm</div>
                    <div style="font-size:1.6rem; font-weight:800; color:#1E293B; margin:4px 0;">{len(grp_agg):,}</div>
                    <div style="font-size:0.78rem; color:#64748B;">Đang theo dõi sắc thái thảo luận</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with col_gkpi4:
                ...
                <div class="dashboard-card" style="margin-bottom:0.75rem; padding:1rem; border-left:4px solid {on_color};">
                    <div style="font-size:0.75rem; font-weight:700; color:#64748B; text-transform:uppercase;">📊 Net Sentiment Toàn Mạng</div>
            ...
            with st.container(border=True):
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; padding:8px 0; border-bottom:2px solid #E2E8F0; font-size:0.8rem; font-weight:700; color:#64748B;">
                    <div style="width:230px;">TRANG / CỘNG ĐỒNG</div>
                ...
                for idx, row in filtered_grp.iterrows():
                    ...
                    <div style="display:flex; align-items:center; justify-content:space-between; padding:10px 0; border-bottom:1px solid #F1F5F9; min-height:46px;">
                        <div style="width:230px; font-size:0.88rem; font-weight:700; color:#1E293B; line-height:1.3; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="{g_name}">
            ...
            with st.container(border=True):
                st.markdown("""
                <div style="font-size:1rem; font-weight:700; color:#1E293B; margin-bottom:8px;">
                    🔍 Chi tiết bài viết & bình luận theo từng Trang / Hội nhóm
                </div>
                """, unsafe_allow_html=True)
                
                drilldown_group_list = filtered_grp['clean_group'].tolist()
                selected_drill_group = st.selectbox(
                    "Chọn Trang / Nhóm để kiểm tra luồng thảo luận gốc:",
                    options=drilldown_group_list,
                    key="select_drill_group"
                )
        ...
        with st.container(border=True):
            ...
            with col_mc_sel:
                selected_macro_ch = st.selectbox(
                    ...
                    index=0,
                    key="macro_ch_filter_select_main"
                )
            with col_mc_stat:
        ...
        with st.container(border=True):
            ...
            if not neg_df.empty:
                ...
                donut_event = st.altair_chart(
                    ...
                    use_container_width=True,
                    on_select="rerun",
                    key=f"neg_donut_chart_d_{neg_donut_ver}"
                )
        ...
        with st.container(border=True):
            ...
            if not neg_df.empty:
                ...
                for idx, r in neg_bars.iterrows():
                    topic_name = r['Topic']
                    buzz_cnt = r['Buzz']
                    pct = int((buzz_cnt / max_buzz) * 100)
                    is_active = (active_neg_topic == topic_name)
                    
                    c_t, c_b = st.columns([1.8, 2.2])
                    with c_t:
                        btn_txt = f"✓ {topic_name}" if is_active else topic_name
                        b_type = "primary" if is_active else "secondary"
                        if st.button(btn_txt, key=f"btn_neg_top_d_{idx}_{topic_name}", type=b_type, use_container_width=True):
                            if is_active:
                                st.session_state['active_neg_topic'] = None
                            else:
                                st.session_state['active_neg_topic'] = topic_name
                            st.rerun()
                    with c_b:
                        bar_col = "#DC2626" if is_active else "#EF4444"
                        st.markdown(f"""
                        <div style="display:flex; align-items:center; height:38px; gap:8px;">
                            <div style="flex-grow:1; background:#F1F5F9; border-radius:6px; height:22px; overflow:hidden;">
                                <div style="width:{pct}%; background:{bar_col}; height:100%; border-radius:6px;"></div>
                            </div>
                            <span style="font-size:0.85rem; font-weight:700; color:#1E293B; min-width:28px; text-align:right;">{buzz_cnt}</span>
                        </div>
                        """, unsafe_allow_html=True)
            else:
    ...
    with st.container(border=True):
        ...
        with c_filter_top:
            selected_feed_topic = st.selectbox(
                ...
                index=def_topic_idx,
                key=f"neg_dd_top_d_{active_neg_topic}"
            )
            if selected_feed_topic != (active_neg_topic or "Tất cả chủ đề"):
                st.session_state['active_neg_topic'] = selected_feed_topic if selected_feed_topic != "Tất cả chủ đề" else None
                st.rerun()
...
# =============================================================
    ...
    active_pos_topic = st.session_state.get('active_pos_topic')
    ...
    with col_pos1:
        with st.container(border=True):
            st.markdown("""
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <span style="font-size:1rem; font-weight:700; color:#1E293B;">Tỷ lệ tích cực theo Trang / Nhóm</span>
            ...
            if not pos_df.empty:
                ...
                donut_event_pos = st.altair_chart(
                    ...
                    use_container_width=True,
                    on_select="rerun",
                    key=f"pos_donut_chart_d_{pos_donut_ver}"
                )
    ...
    # Column 2: Top Chủ đề thảo luận tích cực nhất (Truly Clickable Interactive Bars)
    with col_pos2:
        with st.container(border=True):
            st.markdown("""
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                <span style="font-size:1rem; font-weight:700; color:#1E293B;">Top Chủ đề thảo luận tích cực nhất</span>
                <span style="font-size:0.75rem; color:#059669; background:#ECFDF5; border:1px solid #A7F3D0; padding:2px 8px; border-radius:10px; font-weight:600;">👆 Bấm chủ đề để lọc</span>
            </div>
            """, unsafe_allow_html=True)
            ...
            if not pos_df.empty:
                ...
                max_buzz_pos = pos_bars['Buzz'].max() if not pos_bars.empty else 1
                active_pos_topic = st.session_state.get('active_pos_topic')
                
                for idx, r in pos_bars.iterrows():
                    topic_name = r['Topic']
                    buzz_cnt = r['Buzz']
                    pct = int((buzz_cnt / max_buzz_pos) * 100)
                    is_active = (active_pos_topic == topic_name)
                    
                    c_t, c_b = st.columns([1.8, 2.2])
                    with c_t:
                        btn_txt = f"✓ {topic_name}" if is_active else topic_name
                        b_type = "primary" if is_active else "secondary"
                        if st.button(btn_txt, key=f"btn_pos_top_d_{idx}_{topic_name}", type=b_type, use_container_width=True):
                            if is_active:
                                st.session_state['active_pos_topic'] = None
                            else:
                                st.session_state['active_pos_topic'] = topic_name
                            st.rerun()
                    with c_b:
                        bar_col = "#059669" if is_active else "#10B981"
                        st.markdown(f"""
                        <div style="display:flex; align-items:center; height:38px; gap:8px;">
                            <div style="flex-grow:1; background:#F1F5F9; border-radius:6px; height:22px; overflow:hidden;">
                                <div style="width:{pct}%; background:{bar_col}; height:100%; border-radius:6px;"></div>
    ...
    with st.container(border=True):
        ...
        def_pos_topic_idx = all_pos_topics.index(active_pos_topic) if active_pos_topic and active_pos_topic in all_pos_topics else 0
        def_pos_grp_idx = all_pos_grps.index(active_pos_grp) if active_pos_grp and active_pos_grp in all_pos_grps else 0
        ...
        with c_filter_top:
            selected_feed_topic = st.selectbox(
                ...
                key=f"pos_dd_top_d_{active_pos_topic}"
            )
            if selected_feed_topic != (active_pos_topic or "Tất cả chủ đề"):
                st.session_state['active_pos_topic'] = selected_feed_topic if selected_feed_topic != "Tất cả chủ đề" else None
                st.rerun()
        ...
        with c_filter_grp:
            selected_feed_grp = st.selectbox(
                ...
                key=f"pos_dd_grp_d_{active_pos_grp}"
            )
        ...
        with c_clear_btn:
            st.markdown("<div style='height:28px;'></div>", unsafe_allow_html=True)
            if has_active_filter:
                if st.button("❌ Bỏ lọc", key="btn_clear_pos_filters_d", use_container_width=True):
                    st.session_state['active_pos_topic'] = None
                    st.session_state['active_pos_grp'] = None
                    st.session_state['pos_donut_ver'] = st.session_state.get('pos_donut_ver', 0) + 1
                    st.rerun()
        ...
        display_neg_df = neg_df.copy()
        if active_pos_topic:
            display_pos_df = display_pos_df[display_pos_df['topic_category'] == active_pos_topic]
            display_neg_df = display_neg_df[display_neg_df['topic_category'] == active_pos_topic]
        if active_pos_grp:
            display_pos_df = display_pos_df[display_pos_df['clean_group'] == active_pos_grp]
            display_neg_df = display_neg_df[display_neg_df['clean_group'] == active_pos_grp]

        if has_active_filter:
            filter_tags = []
            if active_pos_topic:
                filter_tags.append(f"Chủ đề: <b style='color:#059669;'>{active_pos_topic}</b>")
            if active_pos_grp:
                filter_tags.append(f"Hội nhóm: <b style='color:#059669;'>{active_pos_grp}</b>")
            st.markdown(f"""
...
# =============================================================
    ...
    # Ensure we inspect Toyota discussions
    toyota_df = df[df['car_model'].astype(str).str.startswith('Toyota')].copy() if not df.empty else pd.DataFrame()
    if toyota_df.empty:
        toyota_df = get_discussions_df(start_date=start_date_arg, end_date=end_date_arg, campaign='Toyota', limit=30000)
        if not toyota_df.empty and 'car_model' in toyota_df.columns:
    ...
    if not toyota_df.empty:
        if 'pain_points' not in toyota_df.columns:
            toyota_df['pain_points'] = toyota_df.apply(detect_pp_record, axis=1)
            toyota_df['pp_count'] = toyota_df['pain_points'].apply(len)
        pp_df = toyota_df[toyota_df['pp_count'] > 0].copy()
    else:
        pp_df = pd.DataFrame()

    total_pp = len(pp_df)
    total_toyota = len(toyota_df)
    ...
    # =========================================================================
    # EXECUTIVE BRIEFING: MOST CRITICAL TOPIC TO CARE ABOUT
    # =========================================================================
    ...
    critical_kw = None
    critical_stat = None
    if not pp_df.empty and kw_counts:
        kw_detailed = {}
        for _, r in pp_df.iterrows():
            sent = str(r.get('sentiment') or '').upper()
            model = str(r.get('car_model') or 'Toyota')
            chan = str(r.get('channel') or 'Mạng xã hội')
            grp = str(r.get('clean_group') or chan)
            for kw in r.get('pain_points', []):
                if kw not in kw_detailed:
                    kw_detailed[kw] = {'total': 0, 'negative': 0, 'models': {}, 'groups': {}}
                kw_detailed[kw]['total'] += 1
                if sent == 'NEGATIVE':
                    kw_detailed[kw]['negative'] += 1
                kw_detailed[kw]['models'][model] = kw_detailed[kw]['models'].get(model, 0) + 1
                kw_detailed[kw]['groups'][grp] = kw_detailed[kw]['groups'].get(grp, 0) + 1

        if kw_detailed:
            # Sort by negative count first (most harmful), then total buzz
            sorted_crit = sorted(kw_detailed.items(), key=lambda x: (x[1]['negative'], x[1]['total']), reverse=True)
            critical_kw, critical_stat = sorted_crit[0]

    if critical_kw and critical_stat:
        c_meta = TOYOTA_PAIN_POINTS.get(critical_kw, {})
        c_name = c_meta.get('name', critical_kw)
        c_pillar = c_meta.get('pillar', 'Chung')
        c_total = critical_stat['total']
        c_neg = critical_stat['negative']
        c_neg_pct = (c_neg / c_total * 100) if c_total > 0 else 0
        top_c_model = max(critical_stat['models'].items(), key=lambda x: x[1])[0] if critical_stat['models'] else 'Chưa xác định'
        top_c_model_cnt = critical_stat['models'].get(top_c_model, 0)
        top_c_grp = max(critical_stat['groups'].items(), key=lambda x: x[1])[0] if critical_stat['groups'] else 'Mạng xã hội'

        if c_pillar == "An toàn & Triệu hồi" or critical_kw in ['lỗi', 'túi khí', 'thùng tôn', 'mỏng', 'cháy', 'gãy', 'kẹt']:
            risk_badge = "RỦI RO KỸ THUẬT & AN TOÀN"
            c_action = f"Phản ánh tiêu cực tập trung vào lỗi kỹ thuật/vận hành liên quan đến <b>{top_c_model}</b> tại <b>{top_c_grp}</b>. Khuyến nghị Bộ phận Kỹ thuật & CSKH chủ động rà soát khiếu nại, chuẩn bị tài liệu kỹ thuật minh bạch và xử lý sớm các thắc mắc của chủ xe để ngăn ngừa rủi ro truyền thông lan rộng."
        elif c_pillar == "Giá bán & Đại lý" or critical_kw in ['kèm lạc', 'ngáo giá', 'đắt', 'cắt giảm']:
            risk_badge = "RỦI RO CHÍNH SÁCH GIÁ & ĐẠI LÝ"
            c_action = f"Người tiêu dùng phản ánh tiêu cực về chính sách giá hoặc phân phối đối với dòng xe <b>{top_c_model}</b>. Cần giám sát hoạt động tư vấn bán hàng của hệ thống đại lý và truyền thông nhấn mạnh giá trị sản phẩm."
        elif c_pillar == "Thái độ & Trải nghiệm" or critical_kw in ['chê', 'thất vọng', 'tẩy chay', 'không thích']:
            risk_badge = "RỦI RO TRẢI NGHIỆM & THƯƠNG HIỆU"
            c_action = f"Tỷ lệ tiêu cực cao phản ánh sự thất vọng về trải nghiệm sản phẩm <b>{top_c_model}</b> tại <b>{top_c_grp}</b>. Khuyến nghị đẩy mạnh nội dung giải đáp thắc mắc và lan tỏa trải nghiệm thực tế tích cực từ các chủ xe."
        else:
            risk_badge = "RỦI RO TRUYỀN THÔNG"
            c_action = f"Chủ đề phát sinh lượng ý kiến tiêu cực lớn nhất đối với <b>{top_c_model}</b> tại <b>{top_c_grp}</b>. Cần theo dõi sát diễn biến thảo luận để kịp thời điều phối nội dung truyền thông phản hồi."

        is_currently_filtered = (active_pp_kw == critical_kw)

        # RENDER EXECUTIVE SUMMARY BOX
        with st.container(border=True):
            crit_c_left, crit_c_right = st.columns([3.8, 1.2])
            with crit_c_left:
                st.markdown(f"""
                <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px; flex-wrap:wrap;">
                    <span style="display:inline-flex; align-items:center; gap:6px; background:#FEF2F2; color:#BE123C; border:1px solid #FECDD3; font-size:0.72rem; font-weight:700; text-transform:uppercase; letter-spacing:0.04em; padding:3px 10px; border-radius:6px;">
                        🚨 VẤN ĐỀ CẦN QUAN TÂM NHẤT • {risk_badge}
                    </span>
                    <span style="font-size:0.8rem; color:#64748B;">Nhóm: <b style="color:#334155;">{c_pillar}</b></span>
                </div>
                <div style="display:flex; align-items:baseline; gap:10px; margin-bottom:12px; flex-wrap:wrap;">
                    <span style="font-size:1.35rem; font-weight:800; color:#0F172A; letter-spacing:-0.02em;">Từ khóa nhạy cảm: <span style="color:#BE123C;">{critical_kw}</span></span>
                    <span style="font-size:0.9rem; font-weight:500; color:#64748B;">({c_name})</span>
                </div>
                """, unsafe_allow_html=True)
            with crit_c_right:
                st.markdown("<div style='margin-top:4px;'></div>", unsafe_allow_html=True)
                if is_currently_filtered:
                    if st.button("✕ Bỏ lọc chủ đề này", key="btn_crit_clear_d", use_container_width=True):
                        st.session_state['active_pp_kw'] = None
                        st.session_state['active_pp_pillar'] = None
                        st.rerun()
                else:
                    if st.button(f"🔍 Xem {c_total:,} thảo luận", key="btn_crit_filter_d", type="primary", use_container_width=True):
                        st.session_state['active_pp_kw'] = critical_kw
                        st.session_state['active_pp_pillar'] = c_pillar
                        st.rerun()

            # 4 METRIC CHIPS INSIDE SUMMARY BOX
            st.markdown(f"""
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:10px; margin-bottom:12px;">
                <div style="background:#F8FAFC; border:1px solid #E2E8F0; border-radius:8px; padding:10px 14px;">
                    <div style="font-size:0.7rem; font-weight:600; color:#64748B; text-transform:uppercase; letter-spacing:0.04em;">Khối lượng phản ánh</div>
                    <div style="font-size:1.18rem; font-weight:800; color:#0F172A; margin-top:2px;">
                        {c_total:,} <span style="font-size:0.78rem; font-weight:500; color:#64748B;">buzz</span>
                    </div>
                </div>
                <div style="background:#FFF1F2; border:1px solid #FFE4E6; border-radius:8px; padding:10px 14px;">
                    <div style="font-size:0.7rem; font-weight:600; color:#BE123C; text-transform:uppercase; letter-spacing:0.04em;">Tỷ lệ tiêu cực</div>
                    <div style="font-size:1.18rem; font-weight:800; color:#9F1239; margin-top:2px;">
                        {c_neg:,} buzz <span style="font-size:0.78rem; font-weight:600; color:#BE123C;">({c_neg_pct:.1f}%)</span>
                    </div>
                </div>
                <div style="background:#EFF6FF; border:1px solid #DBEAFE; border-radius:8px; padding:10px 14px;">
                    <div style="font-size:0.7rem; font-weight:600; color:#1D4ED8; text-transform:uppercase; letter-spacing:0.04em;">Mẫu xe chịu ảnh hưởng nhất</div>
                    <div style="font-size:1.05rem; font-weight:700; color:#1E3A8A; margin-top:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                        {top_c_model} <span style="font-size:0.78rem; font-weight:500; color:#3B82F6;">({top_c_model_cnt} buzz)</span>
                    </div>
                </div>
                <div style="background:#FFF7ED; border:1px solid #FFEDD5; border-radius:8px; padding:10px 14px;">
                    <div style="font-size:0.7rem; font-weight:600; color:#C2410C; text-transform:uppercase; letter-spacing:0.04em;">Tâm điểm thảo luận</div>
                    <div style="font-size:1.05rem; font-weight:700; color:#9A3412; margin-top:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                        {top_c_grp}
                    </div>
                </div>
            </div>
            <div style="background:#FEF2F2; border:1px solid #FECDD3; border-radius:8px; padding:12px 16px; display:flex; align-items:flex-start; gap:10px;">
                <span style="font-size:1.05rem; line-height:1.2;">💡</span>
                <div style="font-size:0.84rem; color:#7F1D1D; line-height:1.5;">
                    <b>Khuyến nghị điều hành:</b> {c_action}
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    active_pp_pillar = st.session_state.get('active_pp_pillar')
    active_pp_kw = st.session_state.get('active_pp_kw')
    pp_reset_ver_d = st.session_state.get('pp_reset_ver_d', 0)
    ...
    with st.container(border=True):
        ...
        if hasattr(st, 'pills'):
            ...
            selected_pill = st.pills(
                ...
                label_visibility="collapsed",
                key=f"pp_pills_sel_d_{pp_reset_ver_d}"
            )
    ...
    with r1_col1:
        with st.container(border=True):
            ...
            if not df_neg_timeline.empty:
                ...
                pp_reset_ver = st.session_state.get('pp_reset_ver_d', 0)
                time_bar_event = st.plotly_chart(
                    fig_pp_timeline,
                    use_container_width=True,
                    on_select="rerun",
                    selection_mode="points",
                    key=f"pp_date_chart_d_{pp_reset_ver}"
                )
                cur_d_pt = None
                if time_bar_event:
                ...
                last_d_pt = st.session_state.get('last_pp_date_pt_d')
                if cur_d_pt != last_d_pt:
                    st.session_state['last_pp_date_pt_d'] = cur_d_pt
        ...
        with st.container(border=True):
            ...
            if not filtered_pp_df.empty:
                ...
                m_bar_event = st.plotly_chart(
                    ...
                    on_select="rerun",
                    selection_mode="points",
                    key=f"pp_model_chart_d_{pp_reset_ver}"
                )
                cur_m_pt = None
                if m_bar_event:
                ...
                last_m_pt = st.session_state.get('last_pp_model_pt_d')
                if cur_m_pt != last_m_pt:
                    st.session_state['last_pp_model_pt_d'] = cur_m_pt
        ...
        with st.container(border=True):
            ...
            if not filtered_pp_df.empty:
                ...
                grp_bar_event = st.plotly_chart(
                    ...
                    on_select="rerun",
                    selection_mode="points",
                    key=f"pp_grp_chart_d_{pp_reset_ver}"
                )
                cur_g_pt = None
                if grp_bar_event:
                ...
                last_g_pt = st.session_state.get('last_pp_grp_pt_d')
                if cur_g_pt != last_g_pt:
                    st.session_state['last_pp_grp_pt_d'] = cur_g_pt
    ...
    with r2_col2:
        with st.container(border=True):
            ...
            if not filtered_pp_df.empty:
                chan_dist = filtered_pp_df['channel'].value_counts().reset_index()
                chan_dist.columns = ['Channel', 'Count']
                chan_dist = chan_dist.sort_values('Count', ascending=True)
                ...
                fig_pp_chan = go.Figure(go.Bar(
                    x=chan_dist['Count'],
                    y=chan_dist['Channel'],
                    orientation='h',
                    marker_color=c_colors,
                    marker_line_color=c_lines,
                    marker_line_width=c_widths,
                ...
                chan_bar_event = st.plotly_chart(
                    ...
                    on_select="rerun",
                    selection_mode="points",
                    key=f"pp_chan_chart_d_{pp_reset_ver}"
                )
                cur_c_pt = None
                if chan_bar_event:
                ...
                last_c_pt = st.session_state.get('last_pp_chan_pt_d')
                if cur_c_pt != last_c_pt:
                    st.session_state['last_pp_chan_pt_d'] = cur_c_pt
    ...
    with st.container(border=True):
        ...
        with feed_c0:
            ...
            if sel_date != (active_pp_date or "Tất cả ngày"):
                ...
                st.session_state['last_pp_date_pt_d'] = new_d
                st.rerun()
        ...
        with feed_c1:
            ...
            if sel_model != (active_pp_model or "Tất cả mẫu xe"):
                ...
                st.session_state['last_pp_model_pt_d'] = new_m
                st.rerun()
        ...
        with feed_c2:
            ...
            if sel_chan != (active_pp_chan or "Tất cả kênh"):
                ...
                st.session_state['last_pp_chan_pt_d'] = new_c
                st.rerun()
        ...
        with feed_c3:
            ...
            if sel_grp != (active_pp_grp or "Tất cả hội nhóm"):
                ...
                st.session_state['last_pp_grp_pt_d'] = new_g
                st.rerun()
        ...
        with feed_c5:
            ...
            if has_any_pp_filter:
                if st.button("✕ Bỏ lọc", key="clear_pp_feed_filters_d", use_container_width=True):
                    ...
                    st.session_state['last_pp_date_pt_d'] = None
                    st.session_state['last_pp_model_pt_d'] = None
                    st.session_state['last_pp_grp_pt_d'] = None
                    st.session_state['last_pp_chan_pt_d'] = None
                    st.session_state['pp_reset_ver_d'] = st.session_state.get('pp_reset_ver_d', 0) + 1
                    st.rerun()
