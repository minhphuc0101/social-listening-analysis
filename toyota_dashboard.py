    ...
    with st.container(border=True):
        ...
        if not neg_df.empty:
            ...
            neg_trend_event = st.plotly_chart(
                ...
                on_select="rerun",
                selection_mode="points",
                key=f"neg_daily_trend_chart_{neg_daily_ver}"
            )
        ...
        with st.container(border=True):
            ...
            if not neg_df.empty:
                ...
                neg_chan_event = st.plotly_chart(
                    ...
                    on_select="rerun",
                    selection_mode="points",
                    key=f"neg_chan_chart_{neg_chan_ver}"
                )
                if neg_chan_event:
            ...
            if not neg_df.empty:
                ...
                for idx, r in neg_bars.iterrows():
                    ...
                    with c_t:
                        ...
                        if st.button(btn_txt, key=f"btn_neg_top_{idx}_{topic_name}", type=b_type, use_container_width=True):
                            if is_active:
    ...
    with st.container(border=True):
        ...
        with c_filter_d:
            selected_feed_date = st.selectbox(
                ...
                index=def_date_idx,
                key=f"neg_dd_date_{active_neg_date}"
            )
        ...
        with c_filter_ch:
            selected_feed_chan = st.selectbox(
                ...
                index=def_chan_idx,
                key=f"neg_dd_chan_{active_neg_channel}"
            )
        ...
        with c_filter_top:
            selected_feed_topic = st.selectbox(
                ...
                index=def_topic_idx,
                key=f"neg_dd_top_{active_neg_topic}"
            )
        ...
        with c_filter_grp:
            selected_feed_grp = st.selectbox(
                ...
                index=def_grp_idx,
                key=f"neg_dd_grp_{active_neg_grp}"
            )
            ...
            if not pos_df.empty:
                ...
                for idx, r in pos_bars.iterrows():
                    ...
                    with c_t:
                        btn_txt = f"✓ {topic_name}" if is_active else topic_name
                        b_type = "primary" if is_active else "secondary"
                        if st.button(btn_txt, key=f"btn_pos_top_{idx}_{topic_name}", type=b_type, use_container_width=True):
                            if is_active:
    ...
    with st.container(border=True):
        ...
        with c_filter_top:
            selected_feed_topic = st.selectbox(
                ...
                key=f"pos_dd_top_{active_pos_topic}"
            )
        ...
        with c_filter_grp:
            selected_feed_grp = st.selectbox(
                ...
                key=f"pos_dd_grp_{active_pos_grp}"
            )
        ...
        with st.container(border=True):
            ...
            with crit_c_right:
                ...
                if is_currently_filtered:
                    if st.button("✕ Bỏ lọc chủ đề này", key="btn_crit_clear", use_container_width=True):
                        st.session_state['active_pp_kw'] = None
                        st.session_state['active_pp_pillar'] = None
                        st.rerun()
                else:
                    if st.button(f"🔍 Xem {c_total:,} thảo luận", key="btn_crit_filter", type="primary", use_container_width=True):
